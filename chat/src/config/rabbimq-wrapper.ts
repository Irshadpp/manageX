...
//       console.log("Chat-Meet Rabbitmq connected!!!");
//     } catch (error) {
...
class RabbitmqWrapper {
    ...
    async connect(): Promise<void> {
        ...
        try {
            this._channel = await connectToRabbitMQ();
            console.log('Chat RabbitMQ connected!');

            // Reconnection logic if the connection is closed
            this._channel.connection.on('close', () => {
                console.log('Chat RabbitMQ connection closed. Reconnecting...');
                this._channel = null;
            ...
            this._isConnecting = false;
            console.error('Chat RabbitMQ connection error:', error);
